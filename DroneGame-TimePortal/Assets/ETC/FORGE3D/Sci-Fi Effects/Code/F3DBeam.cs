...
public class F3DBeam : MonoBehaviour
{
	...
	public GameObject itemParticle;  // itemparticle

	public AudioClip itemUpClip; 
	public float health = 70.0f;
	public bool isStage1HitParticle=false;
	public GameObject Stage1HitParticle;
    ...
    void Awake()
    {
		...
		itemParticle = GameObject.Find ("Item_Blue");
    ...
    void Raycast()
    {
        ...
        {
			if(hitPoint.collider.tag == "ITEMBOX")
			{
				health--;
				//Debug.Log(health);
				if(health < 0 && (isStage1HitParticle == false) ){
					isStage1HitParticle =true;
					Instantiate(Stage1HitParticle,hitPoint.point,Quaternion.identity);
					StartCoroutine(itemUp());
					Destroy(hitPoint.collider);
					Destroy(hitPoint.collider.gameObject, 3.0f);
				}
			}
	...
	IEnumerator itemUp(){
		...
		itemParticle.GetComponent<ParticleSystem> ().startSize = 100;
		AudioSource.PlayClipAtPoint (itemUpClip, transform.position);
