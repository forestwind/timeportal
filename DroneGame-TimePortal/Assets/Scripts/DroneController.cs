...
//			if (GameObject.Find ("GameManager").GetComponent<GameManager> ().CurrentStage != 0) {
...
//
//			
//			roll = Mathf.Floor(roll / 0.01f);	
//			roll *= 0.01f;
//			pitch = Mathf.Floor(pitch / 0.01f);	
//			pitch *= 0.01f;
//			yaw = Mathf.Floor(yaw / 0.01f);	
//			yaw *= 0.01f;
//			gaz = Mathf.Floor(gaz / 0.01f);	
//			gaz *= 0.01f;	
//			gaz+=0.01f;
//
//			if(gaz == 1.01f){
...
//		}
		...
		if(this._ms.isExistMarker(1)){
			...
			// hide Game object
			//GameObject.Find("MarkerObject1").transform.localPosition=new UnityEngine.Vector3(0,0,-2000);
		}
